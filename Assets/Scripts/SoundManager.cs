...
public class SoundManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.Log("SoundManager is already existed!");
            Destroy(gameObject);
        }
    }
    ...
    public void PlaySound(AudioClip audioClip, float v = 1f)
    {
        if (isSoundOn)
        {
            AudioSource.PlayClipAtPoint(audioClip, player.transform.position, v);
        }
    }

    public void PlaySound(AudioClip audioClip,Vector2 pos, float v = 1f)
    {
        if (isSoundOn)
        {
